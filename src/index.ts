...
class RfSwitchPlatform implements DynamicPlatformPlugin {
  ...
  constructor(log: Logging, config: PlatformConfig, api: API) {
    ...
    try {
      this.rfDevice = python.createSync(
        rpiRf,
        'RFDevice',
        gpio,
        1,
      ...
      this.log.error(
        "Error starting rpi-rf, please make sure you've followed the " +
          "installation instructions in this project's readme: " +
          ex
      )
      return
    }
  ...
  didFinishLaunching(): void {
    ...
    if (this.config.devices) {
      this.config.devices.forEach((device: DeviceConfig) => {
        if (!device.name) {
          this.log.error(
            'One of your devices has no name configured. This instance will be skipped.'
          )
        } else if (!device.on_code) {
          this.log.error(
            'One of your devices has no on_code configured. This instance will be skipped.'
          )
        } else if (!device.off_code) {
          this.log.error(
            'One of your devices has no off_code configured. This instance will be skipped.'
          )
        } else {
    ...
    const badAccessories = this.accessories.filter(
  ...
  setService(accessory: PlatformAccessory): void {
    ...
    if (service) {
      const callback: CharacteristicSetCallback = (
        error?: any,
        writeResponse?: any
      ) => {
        if (this.resetTime > 0) {
      ...
      service
        ...
        .on('set', this.setPowerState.bind(this, accessory, callback))
    }
  ...
  nextCommand(): void {
    ...
    const code = todoItem.state
      ? todoItem.accessory.context.on_code
      : todoItem.accessory.context.off_code
    ...
    python
      .call(
        ...
        'tx_code',
        code,
        todoItem.accessory.context.protocol,
        todoItem.accessory.context.pulselength,
        todoItem.accessory.context.codelength
      )
      .then((): void => {
        this.log.debug(
          todoItem.accessory.context.name +
            ' is turned ' +
            (todoItem.state ? 'on.' : 'off.')
        )
